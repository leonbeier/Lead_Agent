...
export const DEFAULT_PREQUALIFICATION_CATEGORY_CONTEXTS: Record<SelectableLeadCategory, EditablePrequalificationCategoryContext> = {
  ...
  integrator_relevant_focus: {
    addOnContext:
      "Require both project-delivery ownership and a relevant vertical such as surveillance, medtech vision, agriculture tech, defence, automotive, industrial automation, semiconductors, embedded systems, measurement automation, or industrial electronics. The vertical should make camera-, inspection-, control-, compute-, or edge-AI use cases plausible. Beyond a plausible vertical, REQUIRE at least one concrete website signal that a Vision-AI fit is real: machine vision, cameras or image processing, optical inspection or quality control, vision brands such as Cognex or Keyence, computer-vision use cases, OR customer-specific software development that is itself tied to machine vision, image processing, optical inspection, visual quality control, or camera/perception data (for example bespoke inspection, image-analysis, or vision-guided automation software). This vision signal is mandatory: generic industrial-automation software, custom control software, process visualization, Prozessvisualisierung, HMI or SCADA dashboards, MES, or plant commissioning do NOT count as a vision signal on their own, because visualizing process data is not machine vision. A company doing only pure PLC/SPS programming, generic automation software, process visualization, commissioning, panel building, electrical installation, or catalog hardware resale without any explicit machine-vision, camera, image-processing, or optical-inspection signal must stay other, not this category. This bucket can include specialist delivery firms in ASIC/FPGA/SoC, embedded, industrial-computing, or instrumentation domains when they build customer-specific technical solutions or integrated systems for clients. Suppliers of embedded-computing platforms, rugged systems, or industrial electronics can also fit when custom solutions and system-integration services are central, not merely catalog hardware sales. Do not use the category only because the company sells robots, drone products, generic engineering capacity, or catalog hardware into one of these verticals. If evidence is mixed between catalog hardware and custom industrial system delivery, prefer this category or other over irrelevant."
  },
  industrial_end_customer_scaled: {
    addOnContext:
      "Treat as relevant only when the company has its own production, operational scale, and a believable quality-control, inspection, or process-automation need. Look for factories, plants, manufacturing lines, engineering teams, or industrial operations. Scale is the decisive gate and the target is a producer at roughly EUR 250 million or more annual revenue. Company websites almost never publish revenue, so judge scale from the proxy evidence they do carry: several named plants, works, Werke or Standorte, more than about 1000 employees, an international production footprint, a group or holding structure with multiple production sites, a portfolio of several brands or product lines, national retail listing or export markets, or stated production volumes and capacities. Never reject an in-scope industrial producer only because a revenue figure is missing, and never reject it for lacking machine-vision, AI, or software evidence - end customers are prospects precisely because they have none of that yet. Any producing industry can qualify when this scale and an in-house production process are evidenced; food and beverage production, printing and packaging printing, and surface, coating or finishing quality control are the verticals with the strongest track record so far and should be treated as the highest-confidence fits rather than as the only allowed ones. Reject small and mid-sized workshops, craft businesses, farm shops, and low-volume producers even when their process would technically fit, and reject pure traders, distributors, agencies, and service providers without their own production."
  },
...
export const OUTREACH_TEMPLATES: Record<string, OutreachTemplate> = {
  integrator_vision_industrial_ai_template: {
    ...
    audience: "Software/automation integrators that already implement Vision AI, computer vision or industrial inspection projects for customers",
    goal: "Position ONE WARE as leverage for integrators solving demanding Vision AI applications where standard models reach their limits.",
  ...
  integrator_general_ai_template: {
    ...
    goal: "Introduce ONE WARE to AI integrators who may not yet do Vision AI, framed around faster, lower-effort Vision AI delivery.",
...
export const CATEGORY_PREQUALIFICATION_CONTEXT: Record<LeadCategory, CategoryPrequalificationContext> = {
  ...
  integrator_relevant_focus: {
    ...
    classificationRules: [
      "Relevant only when the company delivers projects in a relevant vertical (surveillance, defence, medtech vision, robotics, drones, agriculture tech, automotive tech, industrial automation, semiconductors, embedded electronics, or measurement-heavy industrial domains) AND the website shows at least one explicit machine-vision signal: machine vision, cameras or image processing, optical inspection, visual quality control, vision brands such as Cognex or Keyence, or a computer-vision/inspection use case.",
      "A relevant vertical alone is NOT enough. Generic industrial automation, control engineering, process visualization (Prozessvisualisierung), HMI or SCADA dashboards, MES, plant commissioning, robotics motion, or plain custom software development, without any explicit camera, image-processing, optical-inspection, or machine-vision evidence, must be classified as other, not this category. Visualizing process data is not machine vision.",
      "Delivery ownership still matters; do not use this bucket for pure component vendors, generic staff-augmentation engineering firms, or research organizations. Specialist ASIC, FPGA, SoC, embedded-compute, industrial-electronics, or instrumentation consultancies can fit here only when they build customer-specific technical solutions AND a vision, imaging, inspection, or perception signal is present."
    ],
    disqualifiers: ["No project-delivery signals", "Automation, control, process-visualization, HMI/SCADA, commissioning, or generic software-development firm with no explicit machine-vision, camera, image-processing, or optical-inspection evidence (classify as other)", "Irrelevant vertical without any vision or imaging tie-in", "Pure R&D lab with no commercial delivery path"]
  },
  industrial_end_customer_scaled: {
    ...
    classificationRules: [
      ...
      "Scale gate: the target is a producer at roughly EUR 250 million or more annual revenue. Websites rarely publish revenue, so accept equivalent visible evidence instead: several named plants, works, or production sites, more than about 1000 employees, international production or subsidiaries, a group or holding structure, a multi-brand portfolio, national retail listing or export markets, or stated production volumes and capacities. Do not reject an industrial producer merely because no revenue figure is shown; reject it when the visible evidence points to an artisanal, manufaktur, farm-shop, or single-workshop operation.",
